  ...
  try {
    ...
    if (!bodyValidation.success) {
      return res.status(400).json({
        ...
        details: formatValidationError(bodyValidation.error) || 'Invalid listing data',
      });
    }
    
    const createdListing = await listingsService.createListing(req.user.id, bodyValidation.data);
...
export const updateListing = async (req, res, next) => {
  try {
    ...
    if (req.user.role !== 'admin' && existingListing.seller_id !== req.user.id) {
    ...
    const updatedListing = await listingsService.updateListing(id, bodyValidation.data);
