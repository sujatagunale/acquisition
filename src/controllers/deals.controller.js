  ...
  try {
    ...
    if (!paramValidation.success) {
      return res.status(400).json({
        ...
        details: formatValidationError(paramValidation.error) || 'Invalid deal ID',
      });
    }
  ...
  try {
    ...
    if (!bodyValidation.success) {
      return res.status(400).json({
        ...
        details: formatValidationError(bodyValidation.error) || 'Invalid deal data',
      });
    }
    
    const listing = await listingsService.getListingById(bodyValidation.data.listing_id);
    if (!listing) {
      return res.status(404).json({ error: 'Listing not found' });
    }
    
    if (listing.status !== 'listed') {
      return res.status(400).json({ error: 'Listing is not available for deals' });
    }
    
    if (listing.seller_id === req.user.id) {
      return res.status(400).json({ error: 'Cannot create deal on your own listing' });
    }
    
    const createdDeal = await dealsService.createDeal(req.user.id, bodyValidation.data);
  ...
  try {
    ...
    if (!bodyValidation.success) {
      return res.status(400).json({
        ...
        details: formatValidationError(bodyValidation.error) || 'No valid updates provided',
      });
    }
...
export const acceptDeal = async (req, res, next) => {
  try {
    ...
    res.json({
      message: 'Deal accepted successfully. Other deals for this listing have been cancelled.',
      deal: acceptedDeal,
    });
  } catch (error) {
...
export const getDealsByListing = async (req, res, next) => {
  try {
    const paramValidation = dealIdSchema.safeParse({ id: req.params.listingId });
    
    if (!paramValidation.success) {
      return res.status(400).json({
        error: 'Invalid listing ID',
        details: formatValidationError(paramValidation.error) || 'Invalid listing ID',
      });
    }
