...
export async function createDeal(buyerId, dealData) {
  try {
    const [listing] = await db
      .select()
      .from(listings)
      .where(eq(listings.id, dealData.listing_id))
      .limit(1);
    if (!listing) {
...
export async function getDealsByListing(listingId) {
  try {
    ...
    const listingDeals = await db
      ...
      .from(deals)
      .leftJoin(buyer, eq(buyer.id, deals.buyer_id))
      .leftJoin(seller, eq(seller.id, deals.seller_id))
      .where(eq(deals.listing_id, listingId));
